...
Live address: **https://pantryrelay.web.app**. The dedicated site is `pantryrelay`, service is `pantry-relay-api`, and database instance is `pantry-relay-db`, in `us-central1`. The existing hosting project is `gen-lang-client-0444960702`. Google rejected linking another billed project because the account had reached its project quota; these Pantry Relay resources are separate from the other applications in that project. The initially created, unbilled `pantry-relay` project is unused.
...
`scripts/deploy_firebase.sh` deploys the dedicated Pantry Relay service and Hosting site. It reads the database connection URL from Secret Manager, uses a dedicated service account, restricts allowed hosts and browser origins, and caps concurrency and instance count. Existing Cloud SQL, registry, service-account permissions and secrets must be provisioned first. The small database has 10 GB of SSD storage and seven daily backups retained. Cloud Run is limited to two instances with eight requests per instance and scales to zero when idle. Email authentication remains limited to 30 attempts per 15 minutes; the shared-proxy aggregate limit is 300. The script does not create or modify other applications' databases. Firebase routes to the service URL without revision tags, so exact allowed-host checks remain stable. A rollback must deploy the intended Cloud Run image and matching frontend together; Firebase Hosting rollback alone does not roll the API back.
